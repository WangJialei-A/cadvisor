...
function drawCycle(elementId, machineInfo, stats) {
  if (stats.spec.has_cpu && !hasResource(stats, 'perf')) {
    return;
  }
...
if (cur.perf.cycle != undefined) {
    var elements = [];
    elements.push(cur.timestamp);
    elements.push(cur.perf.cycle);
    data.push(elements);}
  }
  drawLineChart(titles, data, elementId, 'Cycles');
}

function drawMemoryLatency(elementId, machineInfo, stats) {
  if (stats.spec.has_cpu && !hasResource(stats, 'perf')) {
    return;
  }
  ...
  for (var i = 1; i < stats.stats.length; i++) {
    ...
    if (cur.perf.l3_miss_requests != undefined) {
    var elements = [];
    elements.push(cur.timestamp);
    elements.push(cur.perf.l3_miss_cycles / cur.perf.l3_miss_requests);
    data.push(elements);}
  }
  drawLineChart(titles, data, elementId, 'Memory Latency');
}

function drawL3MissRequests(elementId, machineInfo, stats) {
  if (stats.spec.has_cpu && !hasResource(stats, 'perf')) {
    return;
  }
  ...
  for (var i = 1; i < stats.stats.length; i++) {
    ...
    var elements = [];
    elements.push(cur.timestamp);
    elements.push(cur.perf.l3_miss_requests);
    data.push(elements);}
  }
  drawLineChart(titles, data, elementId, 'L3 Miss Requests');
}

function drawL3MissCycles(elementId, machineInfo, stats) {
  if (stats.spec.has_cpu && !hasResource(stats, 'perf')) {
    return;
  }
  ...
  for (var i = 1; i < stats.stats.length; i++) {
    ...
    if (cur.perf.l3_miss_cycles != undefined) {
    var elements = [];
    elements.push(cur.timestamp);
    elements.push(cur.perf.l3_miss_cycles);
    data.push(elements);}
  }
  drawLineChart(titles, data, elementId, 'L3 Miss Cycles');
}

function drawCacheMiss(elementId, machineInfo, stats) {
  if (stats.spec.has_cpu && !hasResource(stats, 'perf')) {
    return;
  }
  ...
  for (var i = 1; i < stats.stats.length; i++) {
    ...
    if (cur.perf.cache_miss != undefined) {
    var elements = [];
    elements.push(cur.timestamp);
    elements.push(cur.perf.cache_miss);
    data.push(elements);}
  }
  drawLineChart(titles, data, elementId, 'Cache Misses');
}

function drawInstruction(elementId, machineInfo, stats) {
  if (stats.spec.has_cpu && !hasResource(stats, 'perf')) {
    return;
  }
  ...
  for (var i = 1; i < stats.stats.length; i++) {
    ...
    if (cur.perf.instruction != undefined) {
    var elements = [];
    elements.push(cur.timestamp);
    elements.push(cur.perf.instruction);
    data.push(elements);}
  }
  drawLineChart(titles, data, elementId, 'Instructions');
}

function drawMPKI(elementId, machineInfo, stats) {
  if (stats.spec.has_cpu && !hasResource(stats, 'perf')) {
    return;
  }
  ...
  for (var i = 1; i < stats.stats.length; i++) {
    ...
    var elements = [];
    elements.push(cur.timestamp);
    elements.push(cur.perf.cache_miss * 1000 / cur.perf.instruction);
    data.push(elements);}
  }
  drawLineChart(titles, data, elementId, 'MPKI');
}


function drawCPI(elementId, machineInfo, stats) {
  if (stats.spec.has_cpu && !hasResource(stats, 'perf')) {
    return;
  }
  ...
  for (var i = 1; i < stats.stats.length; i++) {
    ...
    var elements = [];
    elements.push(cur.timestamp);
    elements.push(cur.perf.cycle / cur.perf.instruction);
    data.push(elements);}
  }
  drawLineChart(titles, data, elementId, 'CPI');
}
...
function drawCharts(machineInfo, containerInfo, subcontainers) {
  ...
  // Perf Counter
  steps.push(function () {
    drawCPI('cpi-chart', machineInfo, containerInfo);
  });
  steps.push(function () {
    drawCycle('cycle-chart', machineInfo, containerInfo);
  });
  steps.push(function () {
    drawInstruction('instruction-chart', machineInfo, containerInfo);
  });
  steps.push(function () {
    drawMPKI('mpki-chart', machineInfo, containerInfo);
  });
  steps.push(function () {
    drawCacheMiss('cache-miss-chart', machineInfo, containerInfo);
  });
  steps.push(function () {
    drawL3MissCycles('l3-miss-cycles-chart', machineInfo, containerInfo);
  });
  steps.push(function () {
    drawL3MissRequests('l3-miss-requests-chart', machineInfo, containerInfo);
  });
  steps.push(function () {
    drawMemoryLatency('memory-latency-chart', machineInfo, containerInfo);
  });
